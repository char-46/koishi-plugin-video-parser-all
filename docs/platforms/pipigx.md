...
| 中文名 | 皮皮搞笑 |
| 解析能力 | 短视频 |
| 专属 API | `https://api.bugpk.com/api/pipigx` |
| 备用 API | 不允许 |
...
sequenceDiagram
    ...
    FL->>GPC: getPlatformConfig('pipigx')
    GPC-->>FL: dedicatedFirst=false<br/>专属=pipigx API
    FL->>FA: fetchApi(url)
    Note over FA: apiList = [主API, 专属pipigx]
    FA->>HTTP: GET 主API ?url=
    alt 主API 成功
        HTTP-->>FA: res.data
    else 主API 失败
        FA->>HTTP: GET 专属 pipigx ?url=
        HTTP-->>FA: res.data
    end

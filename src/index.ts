...
export const Config = Schema.intersect([
  ...
  Schema.object({
    ...
    apiKey: Schema.string().default('').description('api-new.ifphp.com API Key'),
    proxy: Schema.object({
  ...
  Schema.object({
    primaryApiUrl: Schema.string().default('https://api-new.ifphp.com/api/svparse').hidden(),
    ...
    customApis: Schema.array(
      Schema.object({
        ...
        ]).default('X-API-Key').description('认证头类型'),
...
function parseApiResponse(raw: any, maxDescLen: number, fieldMapping?: Record<string, string>): ParsedData {
  ...
  const normalizeImage = (img: any): string | null => {
    ...
    if (img && typeof img === 'object' && img.url) {
      return String(img.url).startsWith('http') ? String(img.url) : 'https:' + img.url
    }
    return null
  }
